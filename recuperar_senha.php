    ...
    if (!empty($dados['SendRecupSenha'])) {
        ...
        if (($result_usuario) and ($result_usuario->rowCount() != 0)) {
            ...
            if ($result_up_usuario->execute()) {
                ...
                $link = "atualizar_senha.php?chave=$chave_recuperar_senha";
                ...
                try {
                    ...
                    $mail->isSMTP();
                    $mail->SMTPDebug = 0;
                    $mail->Host       = 'mail.acesv.com.br';
                    $mail->SMTPAuth   = true;
                    $mail->Username   = '_mainaccount@acesv.com.br';
                    $mail->Password   = 'f4b1nh028';
                    $mail->SMTPSecure = 'ssl';
                    $mail->Port       = '465';
                    $mail->setFrom('_mainaccount@acesv.com.br', 'Atendimento');
                    $mail->addAddress($row_usuario['usuario'], $row_usuario['nome']);

                    $mail->isHTML(true);                                  //Set email format to HTML
                    $mail->Subject = 'Recuperar senha';
                    $mail->Body    = 'Prezado(a) ' . $row_usuario['nome'] .".<br><br>Você solicitou alteração de senha.<br><br>Para continuar o processo de recuperação de sua senha, clique no link abaixo ou cole o endereço no seu navegador: <br><br><a href='acesv.com.br/atualizar_senha.php" . $link . "'>" . $link . "</a><br><br>Se você não solicitou essa alteração, nenhuma ação é necessária. Sua senha permanecerá a mesma até que você ative este código.<br><br>";
